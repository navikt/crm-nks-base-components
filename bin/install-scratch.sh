...
insertingTestData() {
   sf data import tree --plan dummy-data/baseData/plan.json || { error $? '"sf data import tree" command failed.'; }
   sf data import tree --plan dummy-data/person/plan.json || { error $? '"sf data import tree" command failed.'; }
...
#!Change this depending on your repo
ORG_ALIAS="crm-nks-base-components"
